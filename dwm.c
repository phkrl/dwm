...
static unsigned int panel[5]={0,0,0,0,0};//positions: 0-bottom panel, 1-top panel, 2-left panel, 3-right panel, 4-empty
	...
	do {
		...
		case MotionNotify:
			...
			&& ny >= selmon->wy && ny <= selmon->wy + selmon->wh) {
				...
				if (abs(selmon->wy + panel[0] - ny) < snap)
					ny = selmon->wy + panel[0];
				else if (abs((selmon->wy + selmon->wh -panel[1] ) - (ny + HEIGHT(c))) < snap)
					ny = selmon->wy + selmon->wh - HEIGHT(c) - panel[1];
...
setup(void)
{
	...
	focus(NULL);
//next: my code
 //       system("conky &"); прожорлив
 //       system("tint2 &");
        system("~/.config/dwm/startup");
...
char* help()
{
	return "usage: dwm [OPTIONS] {VALUE}\noptions:\n-h, --help: show this help\n-v: show version";
}
...
main(int argc, char *argv[])
{
	if (argc == 2 && !strcmp("-v", argv[1]))
		die("dwm-"VERSION "\n");
	else if (argc == 2 && (!strcmp("-h",argv[1]) || !strcmp("--help",argv[1]) ))
		die(help());
