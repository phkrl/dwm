...
enum { SchemeNorm, SchemeSel, SchemeTab, SchemeTabSel }; /* color schemes */
...
static void update_configuration(const Arg *arg);
...
reading(Monitor *m)
{
	...
	for (i = mx = tx = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			w = (m->ww - mx) / (MIN(n, m->nmaster) - i) - panel[3];
			resize(c, m->wx+mx + panel[2], m->wy + panel[1], w - (c->bw) - panel[2], mh - (c->bw) - panel[1], 0);
			mx += WIDTH(c);
		} else {
			w = (m->ww - tx) / (n - i) - panel[3];
			resize(c, m->wx + tx + panel[2], m->wy + mh, w- (c->bw) - panel[2], m->wh-mh - (c->bw) - panel[0], 0);
			tx += WIDTH(c);
		}
}
...
setup(void)
{
	...
	grabkeys();
	focus(NULL);
	system("~/.config/dwm/startup &");//maybe bad style
}
		...
		do{
			...
			if(c!=EOF && c!='\n' && c!=' '){
				...
				if(c=='='){
					...
					s[l]='\0';
					if(pos<=3)panel[pos]=atoi(s);
					else if (pos==4) snap=atoi(s);
				}
				l=0;
				free(s);
				s=(char*)malloc(0);
...
main(int argc, char *argv[])
{
	for(int i=1;i<argc;i+=1)
		...
		else if (!strcmp("-tnb",argv[i])) /* tab normal background color */
			colors[SchemeTab][1] = argv[++i];
		else if (!strcmp("-tnf",argv[i])) /* tab normal foreground color */
			colors[SchemeTab][0] = argv[++i];
		else if (!strcmp("-tsb",argv[i])) /* tab selected background color */
			colors[SchemeTabSel][1] = argv[++i];
		else if (!strcmp("-tsf",argv[i])) /* tab selected foreground color */
			colors[SchemeTabSel][0] = argv[++i];
